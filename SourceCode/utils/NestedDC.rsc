...
Class "NestedDC" (ClassOpts)
    ...
    */
    private macro "CreateLogsumMatrix"(chObj) do
        ...
        NewInfo = {Compressed: 1, FileName: tmpFName, DataType: "double", MatrixLabel: "Person-Cluster Logsum Matrix"}
        opts = {RowIDs: arrPersons, ColIDs: arrClusters, MatrixNames: {"Logsum"}, RowIndexName: "Persons", ColIndexName: "Cluster", NewMatrixInfo: NewInfo}
    ...
    */
    private macro "CreateICMatrix" do
        ...
        arrClusters = v2a(clusterVecs.Cluster)
        obj = CreateObject("Matrix", {Empty: True}) 
        NewInfo = {Compressed: 1, FileName: self.ClassOpts.temp_dir + "/ClusterToCluster.mtx", DataType: "double", MatrixLabel: "Cluster to Cluster Matrix"}
        opts = {RowIDs: arrClusters, ColIDs: arrClusters, MatrixNames: {"Constants"}, RowIndexName: "Cluster", ColIndexName: "Cluster", NewMatrixInfo: NewInfo}
        mObj = obj.CreateFromArrays(opts)
        mObj = null
        mObj = CreateObject("Matrix", self.ClassOpts.temp_dir + "/ClusterToCluster.mtx")
        mObj.Constants := 0
...
/*
    Copy choices file from the choices table written by Nested DC to the ABM Person file
*/
Macro "Copy DC Choices"(opts)
    pView = opts.PrimaryView
    pID = opts.PrimaryViewID
    personField = opts.PersonChoicesField
    choicesFile = opts.ChoicesFile
    idField = opts.IDField
    choiceField = opts.ChoiceField
    
    objC = CreateObject("Table", choicesFile)
    objP = CreateObject("Table", pView)
    objJ = objC.Join({Table: objP, LeftFields: {idField}, RightFields: {pID}})
    objJ.(personField) = objJ.(choiceField)
    objJ = null
    objP = null
    objC = null
endMacro
